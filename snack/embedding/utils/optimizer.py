...
class TSNEMomentum():
    ...
    def momentum_hack_to_quickly_stop_points_going_in_the_wrong_direction(self, dY, iY):
        ...
        self.gains = (self.gains + 0.2) * (selection_mask).double() + (self.gains * 0.8) * (~selection_mask).double()
...
class SGDOptimizer():
    ...
    def take_step_momemtum_gains(self, gradient, iter):
        ...
        #TODO could be a boolean if we want to use this. It is a hack afterall.
        #self.momentum.momentum_hack_to_quickly_stop_points_going_in_the_wrong_direction(self.momentum.gains,self.velocity )
